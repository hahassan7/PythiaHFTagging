...
void bjet_analysis(int nEvents = 10000, DebugLevel debugLevel = DebugLevel::INFO)
{
    ...
    std::vector<std::string> features = {"JetpT", "JetEta", "JetPhi", "JetMass", "JetFlavor",
    ...
    DCASmearing dcaSmearer(debugLevel); // Create instance of DCASmearing
    ...
    for (int iEvent = 0; iEvent < nEvents; ++iEvent)
    {
        ...
        TVector3 truePrimaryVertex(
            pythia.event[5].xProd() * 0.1, // Convert mm to cm
            pythia.event[5].yProd() * 0.1,
            pythia.event[5].zProd() * 0.1);

        // Smear primary vertex
        TVector3 smearedPrimaryVertex = detector.smearPrimaryVertex(truePrimaryVertex);
        ...
        for (int i = 0; i < pythia.event.size(); ++i)
        {
            ...
            detector.smearTrack(part, trackParams, truePrimaryVertex);
        ...
        try
        {
            ...
            try
            {
                ...
                for (const auto &jet : jets)
                {
                    ...
                    std::vector<SecondaryVertexFinder::SecVtxInfo> secondaryVertices = svFinder.findSecondaryVertices(jetTracks);

                    svFinder.fillMonitoringHistograms(secondaryVertices);
